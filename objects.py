...
class spec_config:
    ...
    def run(self, outhandle, errhandle):
        ...
        if not os.path.exists(jar_file):
            errhandle(
                'Failed to locate "specjbb2015.jar" in "{}"'.format(jar_path))
            return 2
        result_dir = "{}/results_{}-runs_{}".format(
            jar_path, len(self.runs),
    ...
    def _rollup(self, path: str, outhandle):
        ...
        outhandle(os.linesep)
        p = Popen(
            shlex.split("{} {} {}".format(perlbin, rollscript, path)),
            cwd=path,
            stdout=PIPE,
            stderr=STDOUT,
        ...
        resname += ".csv"
        result = os.path.join(
            os.path.abspath(os.path.join(path, os.pardir)), resname)
        return result
    ...
    def _totateconfig(self):
        return {
            "TemplateData": {
                "CURSES": {
                    "args": [
                        "JDK", "JVM Options", "Run Type", "Tag", "Numa Nodes",
                        "Verbose", "Report Level", "Skip Report",
                        "Ignore Kit Validation", "Number of Runs",
                        "Data Collection"
                    ],
                    "annotations": {
                        "JDK":
                        "The full path location of a java executable to be invoked",
                        "JVM Options":
                        "A string of arguments to be passed to the JVM",
                        "Run Type":
                        "The run type must be 'composite', 'distributed_ctrl_txl', 'distributed_sut', or 'multi'",
                        "Tag":
                        "The name of this run",
                        "Numa Nodes":
                        "The number of numa nodes to use when running multiple TXINJECTOR's or BACKEND's",
                        "Verbose":
                        "Whether or not SPECjbb will produce verbose output during a run",
                        "Skip Report":
                        "Whether or not SPECjbb will skip producing a report after completion",
                        "Ignore Kit Validation":
                        "Whether or not SPECjbb will perform kit validation prior to running",
                        "Number of Runs":
                        "The number of times this run will execute sequentially",
                        "Data Collection":
...
class spec_encoder(json.JSONEncoder):
    ...
    def default(self, obj):
        if isinstance(obj, spec_config) or isinstance(
                obj, spec_run) or isinstance(obj, props) or isinstance(
                    obj, propitem):
            return obj._tojson()
        ...
        for run in runlist:
            ...
            if temp == '':
                raise AssertionError(
                    "Template '{}' does not exist in TemplateData".format(temp))
...
class spec_run:
    ...
    def __init__(self, fromjson=None):
        if fromjson is None:
            ...
            self.jvm_options = fromjson.get(
                'jvm_options',
                "-Xms29g -Xmx29g -Xmn27g -XX:ParallelGCThreads=48")
            ...
            self.ignore_kit_validation = fromjson.get('ignore_kit_validation',
                                                      False)
    ...
    def _run(self,
             jar_file="",
             result_path="",
             handle_out=_defHandle,
             handle_err=_defHandle):
        """
        ...
        if not os.path.exists(jar_file) or fext != '.jar':
            handle_err(
                'Failed to locate "specjbb2015.jar" in "{}"'.format(jar_file))
            return 2
        ...
        ret = switch[self.run_type](jar_file, result_path, handle_out,
                                    handle_err)
    ...
    def _run_composite(self, jar: str, result_parent: str, handle_out,
        ...
        cmd = '{} {} -jar {} -m COMPOSITE {}'.format(self.jdk, self.jvm_options,
                                                     jar, self._spec_opts())
    ...
    def _run_multi(self, jar: str, result_parent: str, handle_out, handle_err):
        ...
        for x in range(int(self.num_runs)):
            ...
            for g in range(
                ...
                for j in range(self.properties.root[
                        'specjbb.txi.pergroup.count'].value):
                ...
                be_procs.append([
                    p,
                    open(
                        os.path.join(result_dir, '{}.log'.format(be_name)),
                        'w'),
                    open(
                        os.path.join(result_dir, '{}.out'.format(be_name)),
                        'w'),
                    stream(p.stderr),
                    stream(p.stdout)
                ])
    ...
    def _prerun(self, path: str):
        ...
        result_dir = "{}/{}-{}".format(
            path,
            datetime.datetime.fromtimestamp(
                time.time()).strftime('+%y-%m-%d_%H%M%S'), self.tag)
        os.makedirs(result_dir)
        if not os.path.exists("{}/config".format(result_dir)):
            os.makedirs("{}/config".format(result_dir))
        self.properties.writeconfig(
            "{}/config/specjbb2015.props".format(result_dir))
        ...
        shutil.copyfile(templateC,
                        os.path.join(result_dir, "config", "tempalte-C.raw"))
        shutil.copyfile(templateD,
                        os.path.join(result_dir, "config", "tempalte-D.raw"))
        shutil.copyfile(templateM,
                        os.path.join(result_dir, "config", "tempalte-M.raw"))
        return result_dir
    ...
    def _start_data_collection(self, results_dir: str, handle):
        ...
        for c in cmds:
            if c != 'NONE' and c != '':
                shcmd = shlex.split(c)
                if os.path.exists(shcmd[0]) or any(
                        os.access(os.path.join(path, shcmd[0]), os.X_OK)
                        for path in os.environ["PATH"].split(os.pathsep)):
                    handle("Starting data collection ''{}".format(c))
                    stdout = open(
                        os.path.join(results_dir, "{}.log".format(shcmd[0])),
                        'w')
                    procs.append(
                        Popen(
                            shcmd,
                    ...
                    handle(
                        "Failed to start data collection: '{}', command does not exit".
                        format(c))
        return procs
...
class propitem:
    ...
    def _tojson(self):
        ...
        return {"prop": self.prop, "value": self.value}


known_args = [
    'JDK', 'JVM Options', 'Run Type', 'Tag', 'Numa Nodes', 'Verbose',
    'Skip Report', 'Ignore Kit Validation', 'Number of Runs', 'Data Collection'
]

run_types = ['composite', 'distributed_ctrl_txl', 'distributed_sut', 'multi']

loglevels = ['SEVERE', 'WARNING', 'INFO', 'CONFIG', 'FINE', 'FINER', 'FINEST']

con_types = [
    'HBIR_RT', 'HBIR', 'PRESET', 'FIXED_RT', 'FIXED_RT_LOADLEVELS',
    'FIXED_LOADLEVELS'
]
...
def number_validator(x):
    return int(x) <= ord('9') and int(x) >= ord('0')


def float_validator(x):
    return (x <= ord('9') and x >= ord('0')) or x == ord('.')
...
defaults = [
    propitem('specjbb.controller.type', 'HBIR_RT',
             'Controls phases being controlled by Controller.',
             default_validator, lambda x: x in con_types, con_types),
    propitem(
    ...
    propitem(
        ...
        lambda x: int(x) >= 0,
        help_text=
        "Value must an integer. Special '0' value  will force to using the default connectivity provider setting"
    ),
    propitem(
        'specjbb.comm.connect.timeouts.connect',
        60000,
        'Timeout (in milliseconds) for I/O connection operation.',
        number_validator,
        lambda x: int(x) > 0,
        help_text=must_be_positive),
    propitem(
        ...
        60000,
        'Timeout (in milliseconds) for I/O read operation.',
        number_validator,
        lambda x: int(x) > 0,
        help_text=must_be_positive),
    propitem(
        ...
        60000,
        'Timeout (in milliseconds) for I/O write operation.',
        number_validator,
        lambda x: int(x) > 0,
        help_text=must_be_positive),
    propitem(
    ...
    propitem(
        ...
        24000,
        'The network port to which Controller listener will bind.',
        number_validator,
        lambda x: int(x) > 0,
        help_text=must_be_positive),
    propitem(
    ...
    propitem(
        ...
        number_validator,
        lambda x: int(x) >= 0,
        help_text="Value must be greater than or equal to 0"),
    propitem(
        'specjbb.controller.preset.ir',
        1000,
        'Sets IR for preset for controller type ',
        number_validator,
        lambda x: x >= 0,
        help_text="Value must be greater than or equal to 0"),
    propitem(
    ...
    propitem(
        ...
        0,
        'Sets the RT start percent',
        number_validator,
        lambda x: int(x) >= 0 and int(x) <= 100,
        help_text="Value must be between 0 and 100"),
    propitem(
        'specjbb.controller.rtcurve.step',
        0.01,
        'Sets the RT step level to a percent',
        float_validator,
        lambda x: float(x) > 0 and float(x) <= 1,
        help_text=
        "Value must be a decimal greater than 0 and less than or equal to 1"),
    propitem(
    ...
    propitem(
        ...
        number_validator,
        lambda x: int(x) >= 64,
        help_text="Value must be greater than or equal to 64"),
    propitem(
    ...
    propitem(
        ...
        multiprocessing.cpu_count() * 2,
        'Maximum number of worker threads in ForkJoinPool in tier 2 on the Backend side.',
        number_validator,
        lambda x: int(x) > 0,
        help_text=must_be_positive),
    propitem(
        ...
        multiprocessing.cpu_count() * 2,
        'Maximum number of worker threads in ForkJoinPool in tier 3 on the Backend side.',
        number_validator,
        lambda x: int(x) > 0,
        help_text=must_be_positive),
    propitem(
        'specjbb.group.count',
        1,
        'Number of Groups for the run, where Group is TxInjector(s) mapped to Backend.',
        number_validator,
        lambda x: int(x) > 0,
        help_text=must_be_positive),
    propitem(
    ...
    propitem(
        ...
        100000,
        'How much time (in milliseconds) await for heartbeat response from an Agent.',
        number_validator,
        lambda x: int(x) > 0,
        help_text=must_be_positive),
    propitem('specjbb.time.server', False,
             'Enables Controller communication with Time Server.',
             default_validator, lambda x: x is bool, [False, True]),
    propitem(
        'specjbb.txi.pergroup.count',
        1,
        'Number of TxInjectors per Backend in one Group.',
        number_validator,
        lambda x: int(x) > 0,
        help_text=must_be_positive),
    propitem(
        ...
        '.',
        'Directory for storing binary log file of the run.',
        default_validator,
        default_validator,
        help_text="Enter a directory"),
    propitem(
    ...
    propitem(
        ...
        100000,
        ' Number of products in each Supermarket',
        number_validator,
        lambda x: int(x) > 0,
        help_text=must_be_positive),
    propitem('specjbb.logLevel', 'INFO', 'Log level output', default_validator,
             lambda x: x in loglevels, loglevels),
    propitem(
        'specjbb.time.check.interval',
        10000,
        'Time interval (in milliseconds) for periodic time check from Time Server',
        number_validator,
        lambda x: int(x) > 0,
        help_text=must_be_positive),
    propitem(
...
def type_convert(obj):
    ...
    if obj == 'False' or obj == 'false' or obj == 'F' or obj == 'f':
        return False
    try:
...
class props:
    ...
    def get_modified(self):
        ...
        return [
            x for x in list(self.root.values())
            if isinstance(x, propitem) and x.value != x.def_value
        ]
